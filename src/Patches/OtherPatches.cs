...
public static class FindAGameManager_Start
{
    public static void Postfix(FindAGameManager __instance)
    {
        int filterSize = CheatToggles.currentLobbyFilterSize;
        string hostFilter = CheatToggles.playerSearchFilter;

        // Filter für die ausgewählte Spieleranzahl
        if (filterSize > 0)
        {
            __instance.AddIntFilterValue(filterSize, "6", AmongUs.GameOptions.Int32OptionNames.MaxPlayers);
        }
    }
}
...
public static class VersionShower_Start
{
    ...
    public static void Postfix(VersionShower __instance)
    {
        ...
        {
            __instance.text.text =  $"TheAddon v{MalumMenu.malumVersion} (v{Application.version})"; // Supported
        }
        else
        {
            __instance.text.text =  $"TheAddon v{MalumMenu.malumVersion} (<color=red>v{Application.version}</color>)"; // Unsupported
        }
    }
}
...
public static class PingTracker_Update
{
    ...
    public static void Postfix(PingTracker __instance)
    {
        ...
        if (AmongUsClient.Instance.IsGameStarted)
        {
            ...
            __instance.text.text = $"MalumMenu | Modded by TheAddon {pingText}";

            return;
        }

        __instance.text.text = $"MalumMenu | Modded by TheAddon\n{pingText}";
