...
public static class MalumCheats
{
    ...
    private static float _lastCallMeetingTime;
    ...
    public static void InstantGameEndCheat()
    {
        if (!CheatToggles.instantGameEnd) return;

        if (!Utils.isPlayer || !Utils.isInGame) return;

        if (Time.time - _lastCallMeetingTime >= CheatToggles.spamCallMeetingDelay)
        {
            if (Utils.isHost)
            {
                // Same as PlayerControl.ReportDeadBody but without additional checks
            ...
            _lastCallMeetingTime = Time.time;
        }
    }
